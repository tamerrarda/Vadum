...
| OPS-8 | **Linter, formatter and coverage tooling: there is none.** `pnpm verify` checks types, behaviour and the repository's own invariants, but nothing enforces style and no coverage number is produced anywhere. Every option means a new development dependency, and this repository's own rule (`CONTRIBUTING.md`) puts that in front of the owner with a decision record rather than letting a coding session pick one | Nothing is blocked — it is hygiene. It costs a little review attention on every future pull request, and it is the kind of gap a grant reviewer notices | 🟡 OPEN — owner's call, raised 2026-09-12. The rest of the maintenance gap an adviser named is closed on the same day: `pnpm audit --audit-level=moderate` runs in CI (the tree is clean at `low` today), Dependabot updates monthly with `@solana/*` and `@solana-program/*` grouped so D1 cannot be split, `CONTRIBUTING.md` and `.github/CODEOWNERS` exist, and base45 is now tested against RFC 9285's four published vectors and its §6 invalid one instead of only against strings this repository generated. The linter is the one piece left because it cannot be added without choosing between tools — one combined tool, or a separate linter and formatter, or type-aware rules — and in a workspace this size, with `erasableSyntaxOnly` and no `any`, the value is real but small |
...
|---|---|---|---|
| NONCE-9 | Should reconciliation release an abandoned slot by **self-advancing** the nonce rather than reusing the unchanged value? Reuse after the 24 h window means a merchant who submits late — honestly offline for 30 h, or running a modified app that ignores the window — races the payer's next payment on that slot. Self-advancing makes the old payment definitively dead, but costs the payer a ~5,000-lamport fee, i.e. a small SOL float after D26 | Payer capacity versus late-merchant loss | 🟡 OPEN — **non-blocking**: v1 ships the window rule in `23-SPEC`, and the late merchant's loss is stated there. An owner call, not a technical one. **Narrowed 2026-09-12:** advisers priced self-advancing properly — it is not 5,000 lamports but ~890,880 of parked float plus 5,000 per slot, because D38 forces a fee payer to stay rent-exempt *after* its fee, which is ~17% on top of the devnet rent deposit and breaks the zero-SOL-payer claim (D26) to insure at most 25 base units of a third party's goods. Meanwhile the free half shipped: `reserveSlot` now hands a **released** slot out last, least recently released first, because taking the lowest index unconditionally handed the just-released slot straight back and made the late-submission race certain rather than unlikely |
...
| ID | Question | Why it matters | Status |
|---|---|---|---|
| A-1 | Should a **non-transferable** mint (`nonTransferable`) be a `MintBlocker` rather than an `unknown-extension` warning? | Every payment from such a mint fails at execution: the merchant pays the fee and the payer's slot is burned. Today the mint only warns | ✅ RESOLVED 2026-09-12 — `non-transferable` is a `MintBlocker`, `evaluateMint` raises it, and `22-SPEC` and TOK-3 say so. The pausable half is now settled too (2026-09-12): **block while observed `paused: true`, warn on presence, and always set `mutable`**. An adviser found the real defect under the question — `evaluateMint` never read `state.paused`, and `pausableConfig` did not set `mutable`, so a pausable mint with null authorities was classified immutable and under D22 **never went stale**: the most time-varying property in TOK-3 was cached forever. An unreadable pause state warns rather than blocks, deliberately unlike the transfer fee's fail-safe, because a pause can be lifted and a jsonParsed shape change must not take every pausable mint offline. Residual: the jsonParsed field spelling is taken from the program's own `PausableConfig` type and has not been observed on a live mint — none of devnet USDC, mainnet USDC, USDG or PYUSD carries the extension, so the rule affects nothing in production today |
...
Four advisers read the merged repository independently, and a second panel re-read it once every
stream had landed. What they found, and where each is closed:

| ID | Finding | Status |
|---|---|---|
| REV-17 | **T12's published theft bound was false.** It read "the cap once per unspent slot — 100 base units at N=5 — until the payer reconnects", but nothing counted payments across slots being re-armed. `reconcile` re-arms every settled slot and a `NONCE_RETURN` re-arms one offline, so a thief holding an unlocked phone runs the merchant app, settles their own payments, taps "Refresh from the network", and gets five more: the whole token balance in cap-sized steps. Reconnecting was the refill, not the end of the exposure. `theftExposure` and its test multiplied slots by the cap and so pinned the false number, and T12's "the README says so" about the lock screen pointed at a README that said nothing | ✅ RESOLVED 2026-09-13 — a **24-hour spend limit** (100, proposed and not yet signed off) kept by `client/pool.ts`: `reserveSlot(merchant, now, amount)` logs the amount in the same write that marks the slot spent and throws the new `LIMIT_PAYER_ALLOWANCE` past the limit, and `Pool.allowance` lets the confirmation screen say so first. Nothing but time refills it — not `reconcile`, `applyNonceReturn`, `close`, `create` or `recover`, which carry the log over — and a future-dated entry still counts, so winding the clock back refills nothing. T12 now states the bound as 100 per 24 hours by the device clock (800 over a week) and names what defeats it: setting the clock forward, running code in the app's origin, and the absence of any other device from which to move the balance away. The README's honest limits gained the entry. Still open and not touched here: **T5's "remembered merchants" mitigation is implemented nowhere** |
| REV-16 | **RECOVERY was a dead end, and it stranded the refundable deposit.** The screen offered one button, "Restore from the network" → `Pool.refresh`, and every `Pool` method begins by reading the ledger (`pool.ts` `loaded()`), so on the device this state exists for it could only throw `NONCE_LEDGER_MISSING`. `close` threw the same, which meant the deposit D26 advertises as fully refundable was unrecoverable — the user's only escape was clearing site data, after which onboarding's `createAccountWithSeed` fails on accounts that already exist. T4's headline mitigation, "offline signing is blocked until one online session restores pool state", was therefore unimplementable as built. The browser test pinned that the button was *visible*, never that it worked, and `scenarios.test.ts` reached the same state by calling `create` instead | ✅ RESOLVED 2026-09-12 — `Pool.recover` rebuilds the slot list from the derived seeds (possible only because addresses need no local record, D14), skipping any account whose authority has moved away. Recovered slots are **`unknown`**, so the pool can be closed but never spent from: what comes back is the deposit, not the capacity to pay, and the screen says exactly that. `home` also gained a "Create new slots" button, without which a closed or recovered pool was its own dead end. Specced in `26-SPEC`, corrected in T4 and C3 |
| REV-15 | **The payer app could never complete a payment, and 344 green tests could not see it.** `MintCache.refresh` is the only writer of a `mint:` record, and the only callers were the merchant app and `tools/devnet-b` — the payer app called `mints.load()` over a store nothing had ever written. So `checkAgainstCache` threw `MINT_UNKNOWN` for every scanned `INTENT` (`packages/wire/src/codec.ts:155`), with copy reading *"Reconnect once to check it"* and pointing at a screen that did not exist. A fresh device could fund a pool, go offline, scan, and be stuck for good — this is checklist step 5, which no device had run. The suite missed it because every test supplied the cache itself: `packages/integration/test/devices.ts` builds it from fixtures, `apps/e2e/test/merchant.spec.ts` seeds `mint:<addr>` for the *merchant*, and the payer's browser tests never reached a confirmation screen. Root cause is a spec hole, not a coding slip: C0 is pool setup only, C4 assumed a populated cache, and `26-SPEC` never said who calls `refresh` on the payer side | ✅ RESOLVED 2026-09-12 — the payer app owns a **token screen**, reachable from the home screen, from the end of onboarding (the one moment the device is certainly online) and from the `MINT_UNKNOWN` failure itself; it refuses an incompatible mint there rather than at a confirmation screen with a merchant waiting (D8). With no token checked the home screen says so and does not offer to scan a payment. C4 now states who fills the cache, and a browser test seeds a pool ledger — reading the identity's public key back out of IndexedDB — to pin all of it |
| REV-14 | **Nothing bounded what the payer's device would sign, and the threat model had no theft entry.** Every cap lives in `client/queue.ts` and protects a merchant; an unlocked installed payer app could spend the whole token balance for anyone holding it, who simply runs the merchant app and scans their own request. `31-PARAMETERS.md` named a device biometric as the control — which a PWA cannot provide offline — while the app asked for the amount to be typed again, which a thief does happily | ✅ RESOLVED 2026-09-12 — a **payer per-payment cap** (20) refused in the confirmation screen before anything is signed, with the exposure stated as the cap once per unspent slot (100 at N=5) — *a bound that was false, corrected by REV-17*. The threshold row now says what re-entry actually defends against, the EMV CVM mapping is marked weaker than the card case, and the new **T12** carries the threat. C-7 is closed with it: no WebAuthn, because a prompt over a key in IndexedDB looks like hardware backing without being it |
| REV-13 | **"Independently generated reference" claimed more than the fixtures deliver.** The generator imports no `@vadum/*` package — `check:fixtures-deps` enforces that — but it builds its messages with the same `@solana/kit` and `@solana-program/*` versions as `core`, and `check:solana-versions` guarantees there is exactly one copy of each. So the fixtures pin **determinism**: a wrong account ordering inside kit would reproduce identically on both sides and CI would stay green. D3 says this in as many words, sixty kilobytes away from the claim | ✅ RESOLVED 2026-09-12 — the README calls it a second implementation of the spec that shares kit, says what it pins, and points the correctness evidence at Phase 0, where a validator executed those bytes. `00-INDEX.md` says the same where it introduces the vectors |
